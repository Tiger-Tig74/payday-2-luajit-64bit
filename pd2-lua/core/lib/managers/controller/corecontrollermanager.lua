...
function ControllerManager:replace_active_controller(replacement_ctrl_index, replacement_ctrl)
	...
	for wrapper_id, wrapper in pairs(self._controller_wrapper_list) do
		if wrapper:get_type() == "xb1" then
			...
			wrapper:rebind_connections(nil, )
		end
	end
...
function ControllerManager:create_controller(name, index, debug, prio)
	...
	if debug then
		...
		controller_wrapper = CoreControllerWrapperDebug.ControllerWrapperDebug:new(wrapper_list, self, self._next_controller_wrapper_id, name, default_wrapper, CoreControllerWrapperSettings.ControllerWrapperSettings:new(CoreControllerWrapperDebug.ControllerWrapperDebug.TYPE, nil, , ))
	else
