...
function CoreOldWorldDefinition:create_environment(data, offset)
	managers.viewport:set_default_environment(data.environment, nil, )
...
function CoreOldWorldDefinition:add_trigger_sequence(unit, triggers)
	...
	for _, trigger in ipairs(triggers) do
		if is_editor and Global.running_simulation then
			...
			unit:damage():add_trigger_sequence(trigger.name, trigger.notify_unit_sequence, notify_unit, trigger.time, nil, , is_editor)
		elseif self._all_units[trigger.notify_unit_id] then
			unit:damage():add_trigger_sequence(trigger.name, trigger.notify_unit_sequence, self._all_units[trigger.notify_unit_id], trigger.time, nil, , is_editor)
...
function CoreOldWorldDefinition:use_me(unit, is_editor)
	...
	if self._trigger_units[id] then
		for _, t in ipairs(self._trigger_units[id]) do
			t.unit:damage():add_trigger_sequence(t.trigger.name, t.trigger.notify_unit_sequence, unit, t.trigger.time, nil, , is_editor)
		end
	end
...
function CoreEnvironment:create(offset)
	if self._values.environment ~= "none" then
		managers.viewport:set_default_environment(self._values.environment, nil, )
	end
