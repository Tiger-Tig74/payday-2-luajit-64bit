...
function EditUnitTriggable:paste_element()
	if self._copied_trigger_data then
		if alive(self._copied_trigger_data.notify_unit) and self._copied_trigger_data.notify_unit:damage() then
			self._ctrls.unit:damage():add_trigger_sequence(self._copied_trigger_data.trigger_name, self._copied_trigger_data.notify_unit_sequence, self._copied_trigger_data.notify_unit, self._copied_trigger_data.time, nil, , true)
			self:update_element_gui()
		else
...
function EditUnitTriggable:add_unit(unit)
	...
	if #triggable_sequences > 0 then
		self._ctrls.unit:damage():add_trigger_sequence(self._triggers_params.ctrlr:get_value(), triggable_sequences[1], unit, 0, nil, , true)
		self:update_element_gui()
	end
end
