...
local serpent = function ()
	...
	local function deserialize(data, opts)
		...
		local f, res = loadstring or load("return " .. data, nil, , env)

		if not f then
			f, res = loadstring or load(data, nil, , env)
		end
...
local function handle(params, client, options)
	...
	if command == "run" or command == "step" or command == "out" or command == "over" or command == "exit" then
		...
		while true do
			...
			elseif status == "401" then
				...
				return nil, , "Debugger error: unexpected response '" .. breakpoint .. "'"
			end
	...
	elseif command == "eval" or command == "exec" or command == "load" or command == "loadstring" or command == "reload" then
		...
		if exp or command == "reload" then
			...
			elseif command == "loadstring" then
				...
				if not file then
					return nil, , "Cannot open file " .. exp
				end
			...
			while true do
				...
				if not params then
					return nil, , "Debugger connection " .. (err or "error")
				end
				...
				if status == "200" then
					...
					if len > 0 then
						...
						if err then
							...
							return nil, , "Error in processing results: " .. err
						end
				...
				elseif status ~= "202" then
					...
					elseif status == "401" then
						...
						return nil, , "Debugger error: unexpected response after EXEC/LOAD '" .. params .. "'"
					end
				end
	...
	elseif command == "stack" then
		...
		elseif status == "401" then
			...
			return nil, , "Debugger error: unexpected response after STACK"
		end
	elseif command == "output" then
		...
		if stream and mode then
			...
			if not resp then
				...
				return nil, , "Debugger connection error: " .. err
			end
			...
			elseif type(options) == "table" and options.handler then
				...
				return nil, , "Debugger error: can't redirect " .. stream
			end
		else
			print("Invalid command")
		end
	elseif command == "basedir" then
		...
		if dir then
			...
			if status == "200" then
				...
				return nil, , "Debugger error: unexpected response after BASEDIR"
			end
		else
			print(basedir)
		end
