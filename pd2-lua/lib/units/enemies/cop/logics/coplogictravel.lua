...
function CopLogicTravel._upd_enemy_detection(data)
	...
	local my_data = data.internal_data
	local delay = CopLogicBase._upd_attention_obj_detection(data, nil, )
...
function CopLogicTravel.action_complete_clbk(data, action)
	...
	elseif action_type == "dodge" then
		local objective = data.objective
		local allow_trans, obj_failed = CopLogicBase.is_obstructed(data, objective, nil, )
...
function CopLogicTravel._reserve_pos_along_vec(look_pos, wanted_pos)
	...
	local res_data = managers.navigation:reserve_pos(nil, , wanted_pos, step_clbk, 30, data.pos_rsrv_id)
