...
function FPCameraPlayerBase:_update_stance(t, dt)
	if self._shoulder_stance.transition then
		...
		if trans_data.duration < elapsed_t then
			...
			if in_steelsight and not in_second_sight and not self:get_steelsight_swap_state() and trans_data.steelsight_swap_progress_trigger <= absolute_progress then
				self:_set_steelsight_swap_state(true)
...
function FPCameraPlayerBase:spawn_taser_hooks()
	if not alive(self._taser_hooks_unit) and alive(self._parent_unit) then
		...
		self._taser_hooks_unit:anim_play(Idstring("taser_hooks"), 1.5)
