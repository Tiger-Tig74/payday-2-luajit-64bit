...
function GroupAIStateBesiege:_init_misc_data()
	...
	if managers.navigation:is_data_ready() then
		...
		self:create_timed_groups_table()
	end
end
	...
	if nr_wanted > 0 and task_data.phase ~= "fade" then
		...
		if not used_event then
			if next(self._spawning_groups) then
				...
				local spawn_group, spawn_group_type = self:_find_spawn_group_near_area(primary_target_area, self._tweak_data.assault.groups, nil, , )
...
function GroupAIStateBesiege:_upd_recon_tasks()
	...
	if not used_event then
		...
		if next(self._spawning_groups) then
			...
			local spawn_group, spawn_group_type = self:_find_spawn_group_near_area(task_data.target_area, self._tweak_data.recon.groups, nil, , callback(self, self, "_verify_anticipation_spawn_point"))
			...
			if undershot > 0 and not self._task_data.regroup.active and self._task_data.assault.phase ~= "fade" and self._task_data.reenforce.next_dispatch_t < t and self:is_area_safe(task_data.target_area) then
				...
				if not used_event then
					if next(self._spawning_groups) then
						...
						local spawn_group, spawn_group_type = self:_find_spawn_group_near_area(task_data.target_area, self._tweak_data.reenforce.groups, nil, , )
...
function GroupAIStateBesiege:_spawn_phalanx()
	...
	local spawn_group, spawn_group_type = self:_find_spawn_group_near_area(phalanx_area, phalanx_group, nil, , )
...
function GroupAIStateBesiege:create_timed_groups_table()
	...
	for group_id, group_tweak_data in pairs(tweak_data.group_ai.enemy_spawn_groups) do
		if group_tweak_data.spawn_cooldown and (not group_tweak_data.max_nr_simultaneous_groups or group_tweak_data.max_nr_simultaneous_groups > 0) then
			local individual_groups = {}
			timed_groups[group_id] = {
...
function GroupAIStateBesiege:_spawn_timed_group(task_data, group_data, target_area, group_to_allow)
	local spawn_group, spawn_group_type = self:_find_spawn_group_near_area(target_area, group_to_allow, nil, , )
	...
	local group = self:_spawn_in_group(spawn_group, spawn_group_type, grp_objective, task_data)

	if group then
		group_data.group = group
		group.respawn_data = group_data
...
function GroupAIStateBesiege:_respawn_unit_for_group(task_data, group_data, target_area, respawn_data, spawn_unit_type, group_to_allow)
	...
	if spawned_unit then
		if group_data.group and self._groups[group_data.group.id] then
			managers.groupai:state():assign_enemy_to_existing_group(spawned_unit, group_data.group)
		else
			managers.groupai:state():assign_enemy_to_group_ai(spawned_unit, group_data.team_id)
		end
