...
function SavefileManager:update_gui_visibility()
	if self._hide_gui_time and self._hide_gui_time <= self._timer:time() then
		self._workspace:hide()
...
function SavefileManager:_save(slot, cache_only, save_system)
	...
	if is_setting_slot and managers.user.STORE_SETTINGS_ON_PROFILE then
		...
		local task_data = {
			...
			task_type = self.SAVE_TASK_TYPE,
			user_index = managers.user:get_platform_id(),
			subtitle = managers.localization:text(is_setting_slot and "savefile_setting" or "savefile_progress", {
				VERSION = self.LOWEST_COMPATIBLE_VERSION
			}),
			details = managers.localization:text(is_setting_slot and "savefile_setting_description" or "savefile_progress_description"),
			data = {
				meta_data.cache
			}
		}
...
function SavefileManager:_load(slot, cache_only, save_system)
	...
	if cache_only or meta_data.is_synched_cache and meta_data.cache then
		self:_load_done(slot, cache_only, nil, )
	else
		if is_setting_slot then
			self:_set_cache(slot, nil)
		else
		...
		if is_setting_slot and managers.user.STORE_SETTINGS_ON_PROFILE then
			...
			if IS_EPIC then
				if save_system == self.SAVE_SYSTEM_LOCAL then
					clbk_name = "clbk_result_load_backup"
				else
					clbk_name = "clbk_result_load"
				end
			elseif task_data.save_system == self.SAVE_SYSTEM_LOCAL then
				clbk_name = "clbk_result_load_backup"
			else
				clbk_name = "clbk_result_load"
			end
...
function SavefileManager:_set_current_task_type(task_type)
	...
	elseif self._show_gui_time then
		if self._show_gui_time_check < wall_time - self._show_gui_time then
...
function SavefileManager:clbk_result_load_platform_setting_map(task_data, platform_setting_map)
	...
	self:_load_done(self.SETTING_SLOT, false, nil, )
end
