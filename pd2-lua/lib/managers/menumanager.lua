...
function MenuManager:init(is_start_menu)
	...
	self:brightness_changed(nil, , managers.user:get_setting("brightness"))
	self:effect_quality_changed(nil, , managers.user:get_setting("effect_quality"))
	self:fps_limit_changed(nil, , managers.user:get_setting("fps_cap"))
	self:net_packet_throttling_changed(nil, , managers.user:get_setting("net_packet_throttling"))
	self:net_forwarding_changed(nil, , managers.user:get_setting("net_forwarding"))
	self:net_use_compression_changed(nil, , managers.user:get_setting("net_use_compression"))
...
function MenuCallbackHandler:connect_to_host_rpc(item)
	local function f(res)
		...
		elseif res == "JOINED_GAME" then
			...
			managers.network:session():load_level(item:parameters().level_name, nil, , , level_id, nil)
...
function MenuCallbackHandler:host_multiplayer(item)
	...
	managers.network:session():load_level(level_name, nil, , , level_id)
end
...
function MenuPrePlanningInitiator:create_info_items(node, params, selected_item)
	self:create_divider(node, "info_div", nil, , )
...
function MenuPrePlanningInitiator:modifiy_node_preplanning(node, item_name, selected_item)
	...
	for i, data in ipairs(subgroups) do
		if #data.subgroup > 0 then
			...
			if i ~= #subgroups then
				self:create_divider(node, "end_" .. tostring(i), nil, , )
			end
		end
	end
	...
	for index = 1, #tweak_data.preplanning.location_groups do
		...
		if elements then
			...
			if index ~= last_location_index then
				self:create_divider(node, "end_" .. tostring(index), nil, , )
			end
		end
	end
