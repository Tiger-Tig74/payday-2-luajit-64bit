...
function BlackMarketGuiTabItem:select_slot(slot, instant)
	self._slot_selected = math.clamp(self._slot_selected, 1, self._slots and #self._slots or 1)
...
function BlackMarketGui:_get_grenade_stats(name)
	...
	for i, stat in ipairs(self._throwable_stats_shown) do
		...
		if stat.name == "damage" then
			base_stats[stat.name].value = projectile_data and projectile_data.damage * 10
		elseif stat.name == "range" then
			base_stats[stat.name].value = projectile_data and projectile_data.range
...
function BlackMarketGui:damage_falloff_to_string(damage_falloff)
	...
	if optimal_range_distance then
		local has_far_bonus = damage_falloff.far_multiplier > 1
		local has_near_bonus = damage_falloff.near_multiplier > 1

		if has_far_bonus then
		...
		if optimal_range_distance < 1500 then
			if has_far_bonus or has_near_bonus then
		...
		elseif optimal_range_distance < 3000 then
			if has_far_bonus then
				if has_near_bonus then
					return range_bonus .. range_bonus .. range_empty
				end
			...
			if has_near_bonus then
				return range_bonus .. range_filled .. range_empty
			end
			...
			if has_far_bonus then
				if has_near_bonus then
					return range_bonus .. range_filled .. range_bonus
				end
			...
			if has_near_bonus then
				return range_bonus .. range_filled .. range_filled
			end
...
function BlackMarketGui:get_damage_falloff_from_weapon(weapon_id, blueprint)
	local damage_falloff = tweak_data.weapon[weapon_id] and tweak_data.weapon[weapon_id].damage_falloff or nil
...
function BlackMarketGui:show_stats()
	...
	if self._slot_data.dont_compare_stats then
		...
		local base_stats, mods_stats, skill_stats = WeaponDescription._get_stats(self._slot_data.weapon_id, nil, , self._slot_data.default_blueprint)
...
function BlackMarketGui:update_info_text()
	...
	local function update_info_text_invalid_unusable()
		updated_texts[UTS_LOCKED].text = updated_texts[UTS_LOCKED].text .. managers.localization:text("bm_invalid_unusable_kit_hint_short")
		updated_texts[UTS_WARN].text = updated_texts[UTS_WARN].text .. managers.localization:text("bm_invalid_unusable_kit_hint") .. "\n\n"

		for k, v in pairs(slot_data.invalid_unusable_locked) do
			if k == "bad_part_id" or k == "bad_duped_part_id" or k == "bad_materials_id" then
				updated_texts[UTS_WARN].text = updated_texts[UTS_WARN].text .. "» " .. managers.localization:text("bm_invalid_unusable_reason_" .. k) .. "\n"
				local parts_list = nil

				if k == "bad_part_id" then
					parts_list = managers.blackmarket:get_invalid_unusable_parts(slot_data.category, slot_data.slot)
				elseif k == "bad_duped_part_id" then
					parts_list = managers.blackmarket:get_crafted_category_slot(slot_data.category, slot_data.slot)
					parts_list = parts_list and managers.weapon_factory:get_duplicate_parts_by_type(parts_list.blueprint) or {}
				elseif k == "bad_materials_id" then
					parts_list = managers.blackmarket:get_invalid_unusable_mask_blueprint_parts(slot_data.slot, k)
				end

				table.sort(parts_list)

				for i, part_id in ipairs(parts_list) do
					updated_texts[UTS_WARN].text = updated_texts[UTS_WARN].text .. "  '" .. part_id .. "'\n"
				end
			else
				local reason_data = {}
				reason_data.REASON_ID = reason_data.REASON_ID or k == "bad_mask_id" and tostring(slot_data.name)
				reason_data.REASON_ID = reason_data.REASON_ID or k == "bad_textures_id" and tostring(managers.blackmarket:get_crafted_category_slot(slot_data.category, slot_data.slot).blueprint.pattern.id)
				reason_data.REASON_ID = reason_data.REASON_ID or k == "bad_weapon_id" and tostring(slot_data.name)
				reason_data.REASON_ID = reason_data.REASON_ID or k == "bad_weapon_factory_id" and tostring(slot_data.factory_name)
				reason_data.REASON_ID = reason_data.REASON_ID or k == "bad_weapon_factory_mismatch" and tostring(slot_data.name)
				reason_data.REASON_ID2 = reason_data.REASON_ID2 or k == "bad_weapon_factory_mismatch" and tostring(slot_data.factory_name)
				reason_data.REASON_ID = reason_data.REASON_ID or k == "bad_global_value" and tostring(slot_data.name and tweak_data.weapon[slot_data.name] and tweak_data.weapon[slot_data.name].global_value)
				updated_texts[UTS_WARN].text = updated_texts[UTS_WARN].text .. "» " .. managers.localization:text("bm_invalid_unusable_reason_" .. k, reason_data) .. "\n"
			end
		end
	end
	...
	if identifier == self.identifiers.weapon then
		...
		elseif not slot_data.empty_slot then
			updated_texts[UTS_TITLE].text = slot_data.name_localized

			if slot_data.name_color then
				updated_texts[UTS_TITLE].text = "##" .. updated_texts[UTS_TITLE].text .. "##"
				updated_texts[UTS_TITLE].resource_color = {
					slot_data.name_color
				}
			end

			local resource_color = {}
			updated_texts[UTS_STATS].resource_color = resource_color
			local price = slot_data.price or 0

			if price > 0 then
				updated_texts[UTS_STATS].text = "##" .. managers.localization:to_upper_text(slot_data.not_moddable and "st_menu_cost" or "st_menu_value") .. " " .. managers.experience:cash_string(price) .. "##"
			...
			if not slot_data.unlocked then
				if slot_data.lock_text then
					updated_texts[UTS_LOCKED].text = slot_data.lock_text
					updated_texts[UTS_LOCKED].below_stats = true
				else
					...
					updated_texts[UTS_LOCKED].text = text
					updated_texts[UTS_LOCKED].below_stats = true
				end
			...
			if slot_data.last_weapon then
				updated_texts[UTS_WARN].text = updated_texts[UTS_WARN].text .. managers.localization:to_upper_text("bm_menu_last_weapon_warning") .. "\n"
			end

			if slot_data.global_value and slot_data.global_value ~= "normal" then
				local global_value_data = tweak_data.lootdrop.global_values[slot_data.global_value]
				local global_value_desc = managers.localization:to_upper_text(global_value_data and global_value_data.desc_id)
				updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text .. "##" .. global_value_desc .. "##"
				updated_texts[UTS_SELECTED].resource_color = global_value_data and global_value_data.color or Color.red
				updated_texts[UTS_SELECTED].below_stats = true
			end
			...
			if weapon_tweak then
				...
				if slot_data.not_moddable then
					...
					if movement_penalty < 1 then
						...
						updated_texts[UTS_WARN].text = updated_texts[UTS_WARN].text .. managers.localization:to_upper_text("bm_menu_weapon_movement_penalty_info", {
							penalty = penalty_as_string
						})
					end
				end

				updated_texts[UTS_WARN].below_stats = true
			end
		elseif slot_data.locked_slot then
			ignore_lock = true
			updated_texts[UTS_TITLE].text = managers.localization:to_upper_text("bm_menu_locked_weapon_slot")

			if slot_data.cannot_buy then
				updated_texts[UTS_LOCKED].text = slot_data.dlc_locked
			else
				updated_texts[UTS_STATS].text = slot_data.dlc_locked
			end

			updated_texts[UTS_SELECTED].text = managers.localization:text("bm_menu_locked_weapon_slot_desc")
		elseif not slot_data.is_loadout then
			...
			updated_texts[UTS_TITLE].text = prefix .. managers.localization:to_upper_text("bm_menu_btn_buy_new_weapon")
			updated_texts[UTS_SELECTED].text = managers.localization:text("bm_menu_empty_weapon_slot_buy_info")
		end
	elseif identifier == self.identifiers.melee_weapon then
		updated_texts[UTS_TITLE].text = self._slot_data.name_localized
		local melee_weapon_data = tweak_data.blackmarket.melee_weapons[slot_data.name]

		if melee_weapon_data.info_id then
			updated_texts[UTS_STATS].text = managers.localization:text(melee_weapon_data.info_id)
			updated_texts[UTS_STATS].below_stats = true
		end

		if not slot_data.unlocked then
			local td_ld_gv = tweak_data.lootdrop.global_values[slot_data.global_value]
			local skill_based = slot_data.skill_based
			local level_based = slot_data.level and slot_data.level > 0
			local dlc_based = slot_data.dlc_based or td_ld_gv and td_ld_gv.dlc and not managers.dlc:is_dlc_unlocked(slot_data.global_value)
			local skill_text_id = skill_based and (slot_data.skill_name or "bm_menu_skilltree_locked") or false
			local level_text_id = level_based and "bm_menu_level_req" or false
			local dlc_text_id = dlc_based and slot_data.dlc_locked or false
			local text = ""
		...
		updated_texts[UTS_SELECTED].resource_color = {}

		if slot_data.global_value and slot_data.global_value ~= "normal" then
			local td_ld_gv = tweak_data.lootdrop.global_values[slot_data.global_value]
			updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text .. "##" .. managers.localization:to_upper_text(td_ld_gv.desc_id) .. "##"

			table.insert(updated_texts[UTS_SELECTED].resource_color, td_ld_gv.color)
		end
	...
	elseif identifier == self.identifiers.grenade then
		...
		if not slot_data.unlocked then
			...
			if grenade_tweak and grenade_tweak.unlock_id then
				updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text(grenade_tweak.unlock_id)
			else
				local skill_based = slot_data.skill_based
				local level_based = slot_data.level and slot_data.level > 0
		...
		updated_texts[UTS_SELECTED].text = desc_text .. "\n"

		if slot_data.global_value and slot_data.global_value ~= "normal" then
			updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text .. "##" .. managers.localization:to_upper_text(tweak_data.lootdrop.global_values[slot_data.global_value].desc_id) .. "##"

			table.insert(updated_texts[UTS_SELECTED].resource_color, tweak_data.lootdrop.global_values[slot_data.global_value].color)
		end
	...
	elseif identifier == self.identifiers.armor then
		...
		if not self._slot_data.unlocked then
			updated_texts[UTS_LOCKED].text = utf8.to_upper(managers.localization:text(slot_data.level == 0 and (slot_data.skill_name or "bm_menu_skilltree_locked") or "bm_menu_level_req", {
				level = slot_data.level,
		...
		elseif managers.player:has_category_upgrade("player", "damage_to_hot") and not table.contains(tweak_data:get_raw_value("upgrades", "damage_to_hot_data", "armors_allowed") or {}, self._slot_data.name) then
			updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text("bm_menu_disables_damage_to_hot")
			updated_texts[UTS_LOCKED].below_stats = true
		elseif managers.player:has_category_upgrade("player", "armor_health_store_amount") then
			...
			updated_texts[UTS_STATS].text = managers.localization:to_upper_text("bm_menu_armor_max_health_store", {
	...
	elseif identifier == self.identifiers.armor_skins then
		...
		if self._slot_data.equipped then
			updated_texts[UTS_STATS].text = "##" .. managers.localization:to_upper_text("bm_menu_equipped") .. "##"
			updated_texts[UTS_STATS].resource_color = tweak_data.screen_colors.text
		elseif not self._slot_data.cosmetic_unlocked then
			if slot_data.dlc_locked then
				updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text(slot_data.dlc_locked)
			else
				updated_texts[UTS_STATS].text = "##" .. managers.localization:to_upper_text("bm_menu_item_locked") .. "##"
				updated_texts[UTS_STATS].resource_color = tweak_data.screen_colors.important_1
			end
		end
		...
		if self._slot_data.cosmetic_rarity then
			...
			updated_texts[UTS_TITLE].text = "##" .. self._slot_data.name_localized .. "##"
			updated_texts[UTS_TITLE].resource_color = rarity_color
		...
		updated_texts[UTS_SELECTED].text = desc
		updated_texts[UTS_SELECTED].resource_color = desc_colors
		updated_texts[UTS_SELECTED].below_stats = true
	...
	elseif identifier == self.identifiers.player_style then
		...
		if player_style_tweak then
			...
			if not slot_data.unlocked then
				updated_texts[UTS_STATS].text = "##" .. managers.localization:to_upper_text("bm_menu_item_locked") .. "##"
				updated_texts[UTS_STATS].resource_color = tweak_data.screen_colors.important_1
				updated_texts[UTS_LOCKED].text = slot_data.dlc_locked and managers.localization:to_upper_text(slot_data.dlc_locked) or managers.localization:to_upper_text(managers.dlc:get_unavailable_id(slot_data.global_value))
			end

			local desc_id = player_style_tweak.desc_id
			local desc_colors = {}
			updated_texts[UTS_SELECTED].text = desc_id and managers.localization:text(desc_id) or ""
			...
			if #desc_colors == 1 then
				updated_texts[UTS_SELECTED].resource_color = desc_colors[1]
			else
				updated_texts[UTS_SELECTED].resource_color = desc_colors
			end
		end
	...
	elseif identifier == self.identifiers.mask then
		...
		if not slot_data.empty_slot then
			...
			if price > 0 and slot_data.slot ~= 1 then
				updated_texts[UTS_STATS].text = "##" .. managers.localization:to_upper_text("st_menu_value") .. " " .. managers.experience:cash_string(price) .. "##" .. "   "
			...
			if slot_data.num_backs then
				updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. "##" .. managers.localization:to_upper_text("bm_menu_item_amount", {
			...
			if #resource_colors == 1 then
				updated_texts[UTS_STATS].resource_color = resource_colors[1]
			else
				updated_texts[UTS_STATS].resource_color = resource_colors
			end
			...
			elseif slot_data.infamy_lock then
				updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text("menu_infamy_lock_info")
			elseif mask_id and achievement_tracker[mask_id] and (not slot_data.unlocked or type(slot_data.unlocked) == "number" and slot_data.unlocked <= 0) then
				...
				if stat and max_progress > 0 then
					...
					if progress_left > 0 then
						...
						updated_texts[UTS_LOCKED].text = "##" .. managers.localization:text(achievement_data.text_id, {
							progress = progress
						}) .. "##"
						updated_texts[UTS_LOCKED].resource_color = tweak_data.screen_colors.button_stage_2
					end
			...
			elseif achievement_lock_id and (not slot_data.unlocked or type(slot_data.unlocked) == "number" and slot_data.unlocked <= 0) then
				...
				if achievement_visual then
					updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text(achievement_visual.desc_id)

					if achievement_visual.progress then
						updated_texts[UTS_LOCKED].text = updated_texts[UTS_LOCKED].text .. " (" .. tostring(achievement_visual.progress.get()) .. "/" .. tostring(achievement_visual.progress.max) .. ")"
					end
				end
			elseif achievement_milestone_lock_id and (not slot_data.unlocked or type(slot_data.unlocked) == "number" and slot_data.unlocked <= 0) then
				for _, data in ipairs(tweak_data.achievement.milestones) do
					if data.id == achievement_milestone_lock_id then
						updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text("bm_menu_milestone_reward_unlock", {
			...
			elseif managers.dlc:is_content_skirmish_locked("masks", mask_id) and (not slot_data.unlocked or type(slot_data.unlocked) == "number" and slot_data.unlocked <= 0) then
				updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text("bm_menu_skirmish_content_reward")
			elseif managers.dlc:is_content_crimespree_locked("masks", mask_id) and (not slot_data.unlocked or type(slot_data.unlocked) == "number" and slot_data.unlocked <= 0) then
				updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text("bm_menu_crimespree_content_reward")
			end

			if mask_id and mask_id ~= "empty" then
				local desc_id = tweak_data.blackmarket.masks[mask_id].desc_id
				updated_texts[UTS_SELECTED].text = desc_id and managers.localization:text(desc_id) or Application:production_build() and "Add ##desc_id## to ##" .. mask_id .. "## in tweak_data.blackmarket.masks" or ""
				...
				if slot_data.global_value and slot_data.global_value ~= "normal" then
					...
					if gvalue_tweak.desc_id then
						updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text .. "\n##" .. managers.localization:to_upper_text(gvalue_tweak.desc_id) .. "##"
						updated_texts[UTS_SELECTED].resource_color = gvalue_tweak.color
					end
				end
			end
		elseif slot_data.locked_slot then
			ignore_lock = true
			updated_texts[UTS_TITLE].text = managers.localization:to_upper_text("bm_menu_locked_mask_slot")
			...
			updated_texts[UTS_SELECTED].text = managers.localization:text("bm_menu_locked_mask_slot_desc")
		else
			if slot_data.cannot_buy then
				updated_texts[UTS_STATS].text = managers.localization:to_upper_text("bm_menu_empty_mask_slot")
				updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text("bm_menu_no_masks_in_stash_varning")
			else
				local prefix = ""
				...
				updated_texts[UTS_TITLE].text = prefix .. managers.localization:to_upper_text("bm_menu_btn_buy_new_mask")
			end

			updated_texts[UTS_SELECTED].text = managers.localization:text("bm_menu_empty_mask_slot_buy_info")
		end
	elseif identifier == self.identifiers.weapon_mod then
		...
		if price > 0 then
			updated_texts[UTS_STATS].text = "##" .. managers.localization:to_upper_text("st_menu_cost") .. " " .. managers.experience:cash_string(price) .. "##"
		...
		updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. (price > 0 and "   " or "")

		if slot_data.previewing then
			updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. managers.localization:to_upper_text("bm_menu_mod_preview")
		elseif slot_data.free_of_charge then
			updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. (unlocked > 0 and managers.localization:to_upper_text("bm_menu_item_unlocked") or managers.localization:to_upper_text("bm_menu_item_locked"))
		else
			updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. "##" .. managers.localization:to_upper_text("bm_menu_item_amount", {
		...
		if slot_data.install_lock then
			updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text(slot_data.install_lock)
			updated_texts[UTS_LOCKED].below_stats = true
		elseif slot_data.dlc_locked then
			updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text(slot_data.dlc_locked)
		elseif conflicted then
			updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text("bm_menu_conflict", {
		...
		if is_gadget or is_ammo or is_bayonet or is_bipod or is_underbarrel_ammo or has_desc then
			...
			updated_texts[UTS_SELECTED].text = managers.weapon_factory:get_part_desc_by_part_id_from_weapon(part_id, crafted.factory_id, crafted.blueprint)
		end
		...
		if slot_data.global_value and slot_data.global_value ~= "normal" then
			if is_gadget or is_ammo or is_bayonet or is_underbarrel_ammo or has_desc then
				...
				updated_texts[UTS_SELECTED].text = "##" .. managers.localization:to_upper_text(tweak_data.lootdrop.global_values[slot_data.global_value].desc_id) .. "##"
			end
		...
		if zoom_magnification then
			...
			if zoom_magnification > 1 then
				...
				updated_texts[UTS_TITLE].text = updated_texts[UTS_TITLE].text .. "  " .. zoom_level_string
			end
		end
		...
		if perks and table.contains(perks, "bonus") then
			...
			table.insert(updated_texts[UTS_SELECTED].resource_color, tweak_data.screen_colors.text)
		end
		...
		if slot_data.removes and #slot_data.removes > 0 then
			...
			if #removed_mods > 0 then
				updated_texts[UTS_WARN].text = managers.localization:to_upper_text("bm_mod_equip_remove", {
					mod = removed_mods
				})
			end
		elseif forbidden_parts and #forbidden_parts > 0 then
			...
			updated_texts[UTS_WARN].text = text
		end
	elseif identifier == self.identifiers.mask_mod then
		...
		updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. managers.localization:to_upper_text("bm_menu_masks") .. ": " .. self._data.topic_params.mask_name

		if mask_base_price and mask_base_price > 0 then
			updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. " " .. managers.experience:cash_string(mask_base_price)
		end

		updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. "\n"
		local resource_color = {}

		for MMI = MMI_MATERIAL, MMI_COLOR_C do
			if mask_mod_info[MMI].is_good then
				updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. MMI_TEXTS[MMI] .. ": " .. managers.localization:text(mask_mod_info[MMI].text)

				if mask_mod_info[MMI].overwritten then
					updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. " ##(" .. managers.localization:to_upper_text("menu_bm_overwritten") .. ")##"
				...
				updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. MMI_TEXTS[MMI] .. ": " .. "##" .. managers.localization:to_upper_text("menu_bm_not_selected") .. "##" .. "\n"
		...
		if slot_data.global_value then
			...
			updated_texts[UTS_SELECTED].text = "\n" .. managers.localization:to_upper_text("menu_bm_highlighted") .. ": " .. slot_data.name_localized
			...
			if mod_price > 0 then
				updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text .. " " .. managers.experience:cash_string(mod_price)
			else
				updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text
			end

			local item_owned_stock_string = managers.blackmarket:get_item_owned_stock_string(slot_data.category, slot_data.global_value, slot_data.name)
			updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text .. "\n" .. managers.localization:to_upper_text("bm_menu_item_amount", {
				amount = item_owned_stock_string
			})
			updated_texts[UTS_SELECTED].resource_color = {}
			...
			if index == 1 then
				customize_mask_blueprint.material = tbl
			elseif index == 2 then
				customize_mask_blueprint.pattern = tbl
			elseif index == 3 then
				customize_mask_blueprint.color = tbl
			end
			...
			if part_info.override then
				...
				table.insert(updated_texts[UTS_SELECTED].resource_color, tweak_data.screen_colors.risk)
			end
		end

		if price then
			updated_texts[UTS_STATS].text = updated_texts[UTS_STATS].text .. managers.localization:to_upper_text("menu_bm_total_cost", {
		...
		if #resource_color == 1 then
			updated_texts[UTS_STATS].resource_color = resource_color[1]
		else
			updated_texts[UTS_STATS].resource_color = resource_color
		end
		...
		if not managers.blackmarket:can_finish_customize_mask(false) then
			...
			if slot_data.dlc_locked then
				updated_texts[UTS_LOCKED].text = updated_texts[UTS_LOCKED].text .. "\n" .. managers.localization:to_upper_text("bm_menu_missing_to_finalize_mask", {
					missed_mods = list_of_mods
				}) .. "\n"
			else
				updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text("bm_menu_missing_to_finalize_mask", {
					missed_mods = list_of_mods
				}) .. "\n"
			end
		elseif price and managers.money:total() < price then
			if slot_data.dlc_locked then
				...
				updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text("bm_menu_not_enough_cash") .. "\n"
			end
		end
	elseif identifier == self.identifiers.deployable then
		...
		if not self._slot_data.unlocked then
			...
			updated_texts[UTS_LOCKED].text = managers.localization:to_upper_text(text_id, {
				level = slot_data.level,
				SKILL = slot_data.name
			})
			updated_texts[UTS_LOCKED].text = updated_texts[UTS_LOCKED].text .. "\n"
	...
	elseif identifier == self.identifiers.character then
		...
		updated_texts[UTS_SELECTED].text = managers.localization:text(slot_data.name .. "_desc")
	elseif identifier == self.identifiers.weapon_cosmetic then
		updated_texts[UTS_TITLE].text = managers.localization:to_upper_text("bm_menu_steam_item_name", {
			type = managers.localization:text("bm_menu_" .. slot_data.category),
			name = slot_data.name_localized
		})
		updated_texts[UTS_TITLE].resource_color = tweak_data.screen_colors.text

		if slot_data.weapon_id then
			updated_texts[UTS_STATS].text = managers.weapon_factory:get_weapon_name_by_weapon_id(slot_data.weapon_id)
		end
		...
		if slot_data.is_a_color_skin then
			if slot_data.equipped then
				...
				if not slot_data.unlocked then
					...
					if not unlocked then
						updated_texts[UTS_WARN].text = managers.localization:text(gvalue_tweak and gvalue_tweak.unlock_id or managers.dlc:get_unavailable_id(global_value))
					elseif not have_color then
						...
						if achievement and managers.achievment:get_info(achievement) then
							local achievement_visual = tweak_data.achievement.visual[achievement]
							updated_texts[UTS_WARN].text = managers.localization:text(achievement_visual and achievement_visual.desc_id or "achievement_" .. tostring(achievement) .. "_desc" or managers.dlc:get_unavailable_id(global_value))
						else
							updated_texts[UTS_WARN].text = managers.localization:text(managers.dlc:get_unavailable_id(global_value))
						end
					end
				end
				...
				updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text .. name_string .. "\n" .. color_index_string .. "\n" .. quality_string

				table.insert(updated_texts[UTS_SELECTED].resource_color, tweak_data.screen_colors.text)
				table.insert(updated_texts[UTS_SELECTED].resource_color, tweak_data.economy.qualities[cosmetic_quality].color or tweak_data.screen_colors.text)
			else
				updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text .. managers.localization:text("bm_menu_customizable_weapon_color_desc")
			end
		else
			if not slot_data.unlocked then
				local safe = self:get_safe_for_economy_item(slot_data.name)
				...
				updated_texts[UTS_WARN].text = (slot_data.default_blueprint and "" or "\n") .. managers.localization:text(lock_text_id, macros)
			...
			if cosmetic_rarity then
				updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text .. managers.localization:to_upper_text("bm_menu_steam_item_rarity", {
					rarity = managers.localization:text(tweak_data.economy.rarities[cosmetic_rarity].name_id)
				})

				table.insert(updated_texts[UTS_SELECTED].resource_color, tweak_data.economy.rarities[cosmetic_rarity].color or tweak_data.screen_colors.text)
			end

			if cosmetic_quality then
				updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text .. (cosmetic_rarity and "\n" or "") .. managers.localization:to_upper_text("bm_menu_steam_item_quality", {
					quality = managers.localization:text(tweak_data.economy.qualities[cosmetic_quality].name_id)
				})
			...
			if cosmetic_bonus then
				...
				if bonus then
					...
					updated_texts[UTS_SELECTED].text = updated_texts[UTS_SELECTED].text .. ((cosmetic_quality or cosmetic_rarity) and "\n" or "") .. managers.localization:text("dialog_new_tradable_item_bonus", {
	...
	elseif identifier == self.identifiers.inventory_tradable then
		if slot_data.name ~= "empty" then
			...
			if slot_data.category == "weapon_skins" then
				...
				updated_texts[UTS_STATS].text = "##" .. name_string .. "##"

				if slot_data.cosmetic_rarity then
					updated_texts[UTS_STATS].resource_color = tweak_data.economy.rarities[slot_data.cosmetic_rarity].color or tweak_data.screen_colors.text
				end

				updated_texts[UTS_SELECTED].text, updated_texts[UTS_SELECTED].resource_color = InventoryDescription.create_description_item({
			...
			elseif slot_data.category == "armor_skins" then
				...
				if slot_data.cosmetic_rarity then
					updated_texts[UTS_TITLE].resource_color = tweak_data.economy.rarities[slot_data.cosmetic_rarity].color or tweak_data.screen_colors.text
				end

				updated_texts[UTS_STATS].text = managers.localization:text(slot_data.desc_id)
			elseif slot_data.safe_entry then
				...
				updated_texts[UTS_STATS].text = content_text
				updated_texts[UTS_STATS].resource_color = color_ranges
			elseif slot_data.desc_id then
				updated_texts[UTS_STATS].text = managers.localization:text(slot_data.desc_id)
			end
		end
	elseif identifier == self.identifiers.custom and self._data.custom_update_text_info then
	...
	if desc_mini_icons and table.size(desc_mini_icons) > 0 then
		...
		updated_texts[UTS_STATS].text = string.rep("     ", table.size(desc_mini_icons)) .. updated_texts[UTS_STATS].text
	end
...
function BlackMarketGui:input_focus()
	...
	local active_node = managers.menu:active_menu().renderer:active_node_gui().name
	local nodes = {
		"blackmarket_outfit_node",
		"blackmarket_outfit_customize_node",
		"blackmarket_mask_node"
	}

	if managers.menu:is_pc_controller() and table.contains(nodes, active_node) then
		if managers.menu_scene and managers.menu_scene:input_focus() then
			return false
		end
...
function BlackMarketGui:get_lock_icon(data, default)
	...
	local dlc = data.dlc
	local unlock_dlc = data.unlock_dlc
	local dlc_unlocked = not dlc or managers.dlc:is_dlc_unlocked(dlc)
	local should_hide_unavailable = data.hide_unavailable or dlc and managers.dlc:should_hide_unavailable(dlc)
	local global_value_dlc = managers.dlc:global_value_to_dlc(global_value)
	local global_value_data = tweak_data.lootdrop.global_values[global_value]

	if unlock_dlc and not managers.dlc:is_dlc_unlocked(unlock_dlc) then
		local unlock_gv = managers.dlc:dlc_to_global_value(unlock_dlc)

		if unlock_gv then
			global_value_data = tweak_data.lootdrop.global_values[unlock_gv] or global_value_data
		end

		dlc_unlocked = false
	end

	if global_value_data and global_value_data.dlc then
		return should_hide_unavailable and "guis/textures/pd2/skilltree/padlock" or global_value_data.unique_lock_icon or "guis/textures/pd2/lock_dlc"
	end
...
function BlackMarketGui:get_lock_color(data, default)
	...
	local func_based = data.func_based
	local dlc_unlocked = not dlc or managers.dlc:is_dlc_unlocked(dlc)
	local should_hide_unavailable = dlc and managers.dlc:should_hide_unavailable(dlc)
	local global_value_dlc = managers.dlc:global_value_to_dlc(global_value)
	local dlc_data = global_value_dlc and Global.dlc_manager.all_dlc_data[global_value_dlc]
	local dlc_data_valid = not dlc_data or dlc_data.app_id and not dlc_data.external and not dlc_data.source_id
	local global_value_data = tweak_data.lootdrop.global_values[global_value]
	...
	if unlock_dlc and not managers.dlc:is_dlc_unlocked(unlock_dlc) then
		...
		if unlock_gv then
			global_value_data = tweak_data.lootdrop.global_values[unlock_gv]
			dlc_unlocked = false
			global_value_dlc = managers.dlc:global_value_to_dlc(unlock_gv)
			dlc_data = global_value_dlc and Global.dlc_manager.all_dlc_data[global_value_dlc]
			dlc_data_valid = not dlc_data or dlc_data.app_id and not dlc_data.external and not dlc_data.source_id
		end
	end

	if dlc_data_valid and not dlc_unlocked and not should_hide_unavailable then
		return global_value_data and global_value_data.unique_lock_color or tweak_data.screen_colors.dlc_buy_color
	end
...
function BlackMarketGui:populate_characters(data)
	local new_data = {}
	local max_items = self:calc_max_items(math.max(#data, CriminalsManager.get_num_characters()), data.override_slots)
	...
	for i = 1, CriminalsManager.get_num_characters() do
		...
		local hide_unavailable = managers.dlc:should_hide_unavailable(character_table.dlc)
		...
		if character_table and (unlocked or not hide_unavailable) then
			...
			new_data.dlc = character_table.dlc
			new_data.global_value = character_table.global_value or managers.dlc:dlc_to_global_value(new_data.dlc)
			...
			if character_table.locks then
				...
				elseif dlc and not managers.dlc:is_dlc_unlocked(dlc) then
					...
					local unlock_id = tweak_data.lootdrop.global_values[global_value] and tweak_data.lootdrop.global_values[global_value].unlock_id or managers.dlc:get_unavailable_id(global_value)
					new_data.dlc_locked = hide_unavailable and managers.dlc:get_unavailable_id(global_value) or unlock_id
				end
			else
				local unlock_id = new_data.dlc and tweak_data.lootdrop.global_values[new_data.global_value] and tweak_data.lootdrop.global_values[new_data.global_value].unlock_id or managers.dlc:get_unavailable_id(new_data.global_value)
				new_data.dlc_locked = hide_unavailable and managers.dlc:get_unavailable_id(new_data.global_value) or unlock_id
			end
	...
	max_items = self:calc_max_items(#data, data.override_slots)
...
function BlackMarketGui:populate_grenades(data)
	...
	for i, grenades_data in ipairs(sort_data) do
		...
		if g_tweak_data and g_tweak_data.locks then
			...
			elseif dlc and not managers.dlc:is_dlc_unlocked(dlc) then
				new_data.dlc_locked = tweak_data.lootdrop.global_values[dlc] and tweak_data.lootdrop.global_values[dlc].unlock_id or managers.dlc:get_unavailable_id(dlc)
			else
		...
		elseif g_tweak_data and g_tweak_data.dlc then
			...
			new_data.dlc_locked = tweak_data.lootdrop.global_values[g_tweak_data.dlc].unlock_id or managers.dlc:get_unavailable_id(g_tweak_data.dlc)
		else
			new_data.lock_texture = self:get_lock_icon(new_data)
			new_data.dlc_locked = tweak_data.lootdrop.global_values[new_data.global_value].unlock_id or managers.dlc:get_unavailable_id(new_data.global_value)
		end
...
function BlackMarketGui:populate_melee_weapons(data)
	...
	local global_value, hide_unavailable = nil
	...
	for id, d in pairs(Global.blackmarket_manager.melee_weapons) do
		...
		hide_unavailable = managers.dlc:should_hide_unavailable(global_value, true)

		if d.unlocked or d.equipped or hide_unavailable and not managers.dlc:is_dlc_unlocked(global_value) then
...
function BlackMarketGui:populate_deployables(data)
	...
	for i, deployable_data in ipairs(sort_data) do
		...
		if d_tweak_data.dlc then
			new_data.dlc_based = true
			new_data.dlc_locked = tweak_data.lootdrop.global_values[d_tweak_data.dlc].unlock_id or managers.dlc:get_unavailable_id(d_tweak_data.dlc)
...
function BlackMarketGui:populate_player_styles(data)
	...
	for _, player_style in ipairs(tweak_data.blackmarket.player_style_list) do
		...
		if Global.blackmarket_manager.player_styles[player_style] and (not hide_unavailable or not not managers.dlc:is_global_value_unlocked(tweak.global_value)) then
...
function BlackMarketGui:populate_gloves(data)
	...
	for i, glove_id in ipairs(tweak_data.blackmarket.glove_list) do
		...
		if Global.blackmarket_manager.gloves[glove_id] and (not hide_unavailable or not not managers.dlc:is_global_value_unlocked(tweak.global_value)) then
	...
	sort_data = self:get_filtered_search_list(sort_data, tweak_data.blackmarket.gloves, "glove_id")
...
function BlackMarketGui:equip_gloves_callback(data)
	managers.blackmarket:set_equipped_glove_id(data.name)
	managers.blackmarket:release_preloaded_category("gloves")
	self:reload()
end

function BlackMarketGui:_preview_gloves(glove_id, done_clbk)
	managers.blackmarket:view_gloves(glove_id, done_clbk)
end

function BlackMarketGui:preview_gloves_callback(data)
	self:_preview_gloves(data.name, callback(self, self, "reload"))
end
...
function BlackMarketGui:populate_masks_new(data)
	...
	local guis_catalog = "guis/"
	local mask_tweak, dlc_global_value, dlc_global_value_tweak, dlc_unlock_id, hide_unavailable, crafted = nil
	...
	for i, index in pairs(data.on_create_data) do
		...
		if crafted then
			...
			mask_tweak = tweak_data.blackmarket.masks[guis_mask_id]
			new_data.name_localized = managers.blackmarket:get_mask_name_by_category_slot("masks", index)
			new_data.raw_name_localized = managers.localization:text(mask_tweak.name_id)
			new_data.dlc = mask_tweak.dlc or managers.dlc:global_value_to_dlc(new_data.global_value)
			new_data.unlock_dlc = mask_tweak.unlock_dlc or new_data.dlc
			...
			if new_data.dlc and not managers.dlc:is_dlc_unlocked(new_data.dlc) then
				new_data.unlocked = false
				new_data.lock_texture = self:get_lock_icon(new_data, "guis/textures/pd2/lock_incompatible")
				dlc_global_value = managers.dlc:dlc_to_global_value(new_data.unlock_dlc)
				dlc_global_value_tweak = dlc_global_value and tweak_data.lootdrop.global_values[dlc_global_value]
				dlc_unlock_id = dlc_global_value_tweak and tweak_data.lootdrop.global_values[dlc_global_value].unlock_id or managers.dlc:get_unavailable_id(dlc_global_value)
				hide_unavailable = managers.dlc:should_hide_unavailable(new_data.dlc)
				new_data.dlc_locked = hide_unavailable and managers.dlc:get_unavailable_id(dlc_global_value) or dlc_unlock_id
			...
			if new_data.unlocked then
				...
				local default_blueprint = mask_tweak.default_blueprint or {}
			...
			if crafted.modded then
				...
				local w = 48
				local h = 48
				local texture_data = tweak_data.blackmarket.textures[pattern]

				table.insert(new_data.mini_icons, {
					layer = 1,
					stream = true,
					texture = texture_data and texture_data.texture or "units/payday2/masks/shared_textures/patterns/pattern_no_color_no_material_df",
					right = right,
					top = top,
					w = w,
					h = h,
...
function BlackMarketGui:populate_weapon_category_new(data)
	...
	local weapon_data = Global.blackmarket_manager.weapons
	local dlc_global_value, dlc_global_value_tweak, dlc_unlock_id, hide_unavailable, locked_global_value = nil
	local new_data = {}
	...
	for i, index in pairs(data.on_create_data) do
		...
		if crafted then
			local crafted_weapon_data = weapon_data[crafted.weapon_id]
			new_data = {
				category = category,
				slot = index,
				name = crafted.weapon_id,
				name_localized = managers.blackmarket:get_weapon_name_by_category_slot(category, index),
				factory_name = crafted.factory_id,
				raw_name_localized = crafted.factory_id and managers.weapon_factory:get_weapon_name_by_factory_id(crafted.factory_id),
				can_afford = true,
				price = managers.money:get_weapon_slot_sell_value(category, index),
			...
			unlocked, part_dlc_lock = managers.blackmarket:weapon_unlocked_by_crafted(category, new_data.slot)
			new_data.unlocked = unlocked
			new_data.level = managers.blackmarket:weapon_level(crafted.weapon_id)
			new_data.equipped = crafted.equipped
			new_data.skill_based = crafted_weapon_data and crafted_weapon_data.skill_based
			new_data.skill_name = new_data.skill_based and "bm_menu_skill_locked_" .. new_data.name
			new_data.func_based = crafted_weapon_data and crafted_weapon_data.func_based
			local bitmap_texture, bg_texture = managers.blackmarket:get_weapon_icon_path(crafted.weapon_id, crafted.cosmetics)
			new_data.bitmap_texture = bitmap_texture
			new_data.bg_texture = bg_texture
			...
			if _G.IS_VR then
				local vr_is_locked = tweak_data.vr:is_locked("weapons", crafted.weapon_id)
				new_data.vr_locked = vr_is_locked
				new_data.unlocked = new_data.unlocked and not vr_is_locked
			end

			new_data.global_value = tweak_data.weapon[new_data.name] and tweak_data.weapon[new_data.name].global_value or "normal"
			new_data.dlc = tweak_data.weapon[crafted.weapon_id] and tweak_data.weapon[crafted.weapon_id].dlc or managers.dlc:global_value_to_dlc(new_data.global_value)
			new_data.unlock_dlc = tweak_data.weapon[crafted.weapon_id] and tweak_data.weapon[crafted.weapon_id].unlock_dlc or new_data.dlc
			dlc_global_value = managers.dlc:dlc_to_global_value(new_data.unlock_dlc)
			locked_global_value = part_dlc_lock or dlc_global_value or "normal"
			dlc_unlock_id = tweak_data.lootdrop.global_values[locked_global_value] and tweak_data.lootdrop.global_values[locked_global_value].unlock_id or part_dlc_lock or nil
			hide_unavailable = managers.dlc:should_hide_unavailable(new_data.dlc)
			...
			new_data.lock_texture = new_data.part_dlc_lock and "guis/textures/pd2/lock_incompatible" or new_data.ignore_slot or self:get_lock_icon(new_data)
			...
			if not new_data.ignore_slot and icon_list then
			...
			if not new_data.ignore_slot then
				...
				elseif currently_holding then
					...
					if data.allow_skinning ~= false and has_mods and active then
						local excluded_weapons = managers.blackmarket:skin_editor():get_excluded_weapons()

						if managers.workshop and managers.workshop:enabled() and not table.contains(excluded_weapons, new_data.name) then
							table.insert(new_data, "w_skin")
						end
					end
					...
					if data.allow_move ~= false and new_data.equipped and new_data.unlocked then
						table.insert(new_data, "w_move")
					end
...
function BlackMarketGui:populate_melee_weapons_new(data)
	...
	for i, melee_weapon_data in ipairs(data.on_create_data) do
		...
		new_data.stream = true
		new_data.global_value = tweak_data.lootdrop.global_values[m_tweak_data.dlc] and m_tweak_data.dlc or "normal"
		new_data.dlc = tweak_data.blackmarket.melee_weapons[new_data.name].dlc or managers.dlc:global_value_to_dlc(new_data.global_value)
		new_data.unlock_dlc = tweak_data.blackmarket.melee_weapons[new_data.name].unlock_dlc or new_data.dlc
		new_data.skill_based = melee_weapon_data[2].skill_based
		new_data.skill_name = "bm_menu_skill_locked_" .. new_data.name
		new_data.func_based = melee_weapon_data[2].func_based
		...
		if not new_data.unlocked then
			...
			if m_tweak_data and m_tweak_data.locks then
				...
				elseif dlc and not managers.dlc:is_dlc_unlocked(dlc) then
					new_data.dlc_locked = tweak_data.lootdrop.global_values[dlc] and tweak_data.lootdrop.global_values[dlc].unlock_id or managers.dlc:get_unavailable_id(dlc)
					local gv = managers.dlc:dlc_to_global_value(dlc)
					local gv_tweak = tweak_data.lootdrop.global_values[gv]
					new_data.lock_texture = gv_tweak and gv_tweak.unique_lock_icon or "guis/textures/pd2/lock_dlc"
				else
...
function BlackMarketGui:populate_weapon_cosmetics(data)
	...
	local cosmetic_data, new_data, bitmap_texture, bg_texture, cosmetic_data, cosmetic_id = nil
	local index_i = 1
	...
	index_i = index_i + 1
	local cosmetics_instances_grouped = {}
	local quality, bonus, equipped, instance_data = nil

	for _, instance_id in ipairs(cosmetics_instances) do
		instance_data = inventory_tradable[instance_id]
		cosmetic_id = instance_data.entry
		quality = instance_data.quality
		bonus = instance_data.bonus
		local key = string.format("%s.%s.%d", cosmetic_id, quality, bonus and 1 or 0)
		cosmetics_instances_grouped[key] = cosmetics_instances_grouped[key] or {
			cosmetic_id = cosmetic_id,
			quality = quality,
			bonus = bonus
		}

		table.insert(cosmetics_instances_grouped[key], instance_id)
	end

	local cosmetic_keys = table.map_keys(cosmetics_instances_grouped)

	for _, key in ipairs(cosmetic_keys) do
		table.sort(cosmetics_instances_grouped[key])
	end

	for _, key in ipairs(cosmetic_keys) do
		instance_data = cosmetics_instances_grouped[key]
		cosmetic_id = instance_data.cosmetic_id
		quality = instance_data.quality
		bonus = instance_data.bonus
		equipped = crafted and crafted.cosmetics and table.contains(instance_data, crafted.cosmetics.instance_id)
		new_data = make_cosmetic_data(data, cosmetic_id, true, quality, bonus, equipped)
		new_data.name = instance_data[1]
		new_data.unlocked = #instance_data

		if new_data.cosmetic_bonus then
			local bonuses = tweak_data.economy:get_bonus_icons(cosmetics_data[cosmetic_id].bonus)
			local x = 0
...
function BlackMarketGui:populate_mods(data)
	...
	for i, c_mod in ipairs(cosmetics_blueprint) do
		if Idstring(weapon_factory_tweak[c_mod].type) == ids_id then
	...
	for i, mod_t in ipairs(data.on_create_data) do
		...
		if mod_name then
			...
			local show_stats = not new_data.conflict and new_data.unlocked and not new_data.dlc_locked and weapon_factory_tweak[new_data.name].type ~= "charm"
	...
	local function update_equipped()
		if equipped then
			...
			for i = 1, #equipped_data do
				table.remove(equipped_data, 1)
			end
			...
			if not is_customize_locked then
				...
				if not equipped_data.is_internal then
					...
					if managers.blackmarket:is_previewing_mod(equipped_data.name) then
						table.insert(equipped_data, "wm_remove_preview")
					elseif not preview_forbidden then
						table.insert(equipped_data, "wm_preview_mod")
					end
				else
				...
				if colors then
					...
					if gmod_td.sub_type then
						table.insert(equipped_data.mini_colors, {
							...
							alpha = tweak_data.custom_colors.defaults.laser_alpha
						})
					end
...
function BlackMarketGui:populate_buy_weapon(data)
	...
	local weapon_list = data.on_create_data
	weapon_list = self:get_filtered_search_list(weapon_list, tweak_data.weapon, "weapon_id")
	...
	local new_data = {}
	local weapon_tweak, dlc_global_value, dlc_global_value_tweak, dlc_unlock_id, hide_unavailable = nil
	local guis_catalog = "guis/"

	for i = 1, #weapon_list do
		local weapon_data = weapon_list[i]
		new_data = {
		...
		weapon_tweak = tweak_data.weapon[new_data.name]
		new_data.equipped = false
		local texture_name = weapon_tweak.texture_name or tostring(new_data.name)
		local bitmap_texture, bg_texture = managers.blackmarket:get_weapon_icon_path(weapon_data.weapon_id, nil)
		new_data.bitmap_texture = bitmap_texture
		new_data.comparision_data = deep_clone(weapon_tweak.stats)
		...
		new_data.global_value = weapon_tweak and weapon_tweak.global_value or "normal"
		new_data.akimbo_gui_data = weapon_tweak and weapon_tweak.akimbo_gui_data
		new_data.dlc = weapon_tweak and weapon_tweak.dlc or managers.dlc:global_value_to_dlc(new_data.global_value)
		new_data.unlock_dlc = weapon_tweak and weapon_tweak.unlock_dlc or new_data.dlc
		dlc_global_value = managers.dlc:dlc_to_global_value(new_data.unlock_dlc)
		dlc_global_value_tweak = dlc_global_value and tweak_data.lootdrop.global_values[dlc_global_value]
		dlc_unlock_id = dlc_global_value_tweak and tweak_data.lootdrop.global_values[dlc_global_value].unlock_id or managers.dlc:get_unavailable_id(new_data.global_value)
		hide_unavailable = managers.dlc:should_hide_unavailable(new_data.dlc)
		...
		local active = true
		local dlc_data = Global.dlc_manager.all_dlc_data[new_data.unlock_dlc]

		if new_data.unlocked then
			if new_data.can_afford and active then
				table.insert(new_data, "bw_buy")
			end
		elseif dlc_data and dlc_data.app_id and not dlc_data.external and not managers.dlc:is_dlc_unlocked(new_data.dlc) and not hide_unavailable then
			table.insert(new_data, "bw_buy_dlc")
		end
		...
		local got_mods = managers.blackmarket:got_new_drop("normal", new_data.category, weapon_list[i].factory_id)
...
function BlackMarketGui:get_filtered_search_list(item_list, td, id_identifier)
	if self._data.search_string then
		local filtered_list = {}
		...
		for i = 1, #item_list do
			...
			if found_id then
				table.insert(filtered_list, item_list[i])
			end
		end
...
function BlackMarketGui:populate_buy_mask(data)
	...
	local mask_tweak, dlc_global_value, dlc_global_value_tweak, dlc_unlock_id, hide_unavailable = nil
	...
	for i = 1, max_masks do
		...
		new_data = {
			...
			category = data.category,
			slot = data.prev_node_data and data.prev_node_data.slot,
			global_value = mask_list[i].global_value,
			global_value_category = data.name
		}
		new_data.dlc = mask_tweak.dlc or managers.dlc:global_value_to_dlc(new_data.global_value)
		new_data.unlock_dlc = mask_tweak.unlock_dlc or new_data.dlc
		...
		dlc_global_value, dlc_global_value_tweak, dlc_unlock_id, hide_unavailable = nil
		...
		if mask_tweak.infamy_lock then
			local infamy_lock = mask_tweak.infamy_lock
		...
		if new_data.unlocked and new_data.unlocked > 0 then
			...
			if dlc_global_value_tweak and dlc_data and dlc_data.app_id and not dlc_data.external and not hide_unavailable then
				table.insert(new_data, "bw_buy_dlc")
			end
...
function BlackMarketGui:populate_mask_mod_types(data)
	...
	for type, mods in pairs(data.on_create_data) do
		...
		if managers.blackmarket:can_finish_customize_mask(false) then
			table.insert(new_data, "mm_preview")
		end
...
function BlackMarketGui:create_steam_inventory(data)
	...
	if table.size(inventory_tradable) > 25 then
		slots_h = 5
	end
	...
	for index, instance_id in ipairs(data.on_create_data) do
		...
		if instance_data then
			...
			if new_data.amount and new_data.amount > 1 then
				new_data.corner_text = {
					selected_text = tostring(new_data.amount) .. "x",
					selected_color = tweak_data.screen_colors.text,
					noselected_text = tostring(new_data.amount) .. "x",
			...
			if instance_data.category == "weapon_skins" then
				...
				if td then
					...
					if data.weapons_with_cosmetics_instance[instance_id] then
						new_data.mini_icons = new_data.mini_icons or {}
...
function BlackMarketGui:choose_weapon_mods_callback(data)
	...
	all_cosmetics = table.filter(all_cosmetics, function (weapon_skin, skin_id)
		return weapon_skin.is_marketable ~= false
	end)
...
function MenuCallbackHandler:reset_character_outfit()
	...
	managers.blackmarket:release_preloaded_category("player_style")
	managers.menu_scene:set_character_gloves(managers.blackmarket:equipped_glove_id())
	managers.blackmarket:release_preloaded_category("gloves")
end
	...
	managers.menu:open_node("inventory_tradable_container_preview_node", {
		{
			...
			pattern_scale = self._slot_data.cosmetic_pattern_scale
		}
	})
...
function BlackMarketGui:_weapon_cosmetics_callback(data, add, yes_clbk)
	...
	local params = {
		...
		customize_locked = add and data.locked_cosmetics or false,
...
function BlackMarketGui:remove_mask_callback(data)
	...
	for category, part in pairs(blueprint) do
		local converted_category = part_name_converter[category] or category
...
function BlackMarketGui:open_weapon_buy_menu(data, check_allowed_item_func)
	...
	for category, items in pairs(item_categories) do
		...
		for _, item in ipairs(items) do
			...
			local sn = gv and tweak_data.lootdrop.global_values[gv] and tweak_data.lootdrop.global_values[gv].sort_number or 0
...
function BlackMarketGui:choose_mask_buy_callback(data)
	...
	for mask_id, mask in pairs(masks_data) do
		if mask.pc or mask.pcs then
			...
			elseif mask.dlc or mask.dlcs then
				...
				local add_dlc = false
				local dlc_tweak, hide_unavailable, global_value = nil

				for _, dlc in ipairs(dlcs) do
					global_value = managers.dlc:dlc_to_global_value(dlc) or mask.global_value or dlc
					hide_unavailable = managers.dlc:should_hide_unavailable(global_value, true)
					add_dlc = not hide_unavailable or managers.dlc:is_dlc_unlocked(dlc)

					if add_dlc then
						table.insert(global_values, mask.global_value or global_value)
	...
	local saved_locs = {}
	local sort_cached_mask_data = {}

	local function cache_mask_data(item)
		local mask_id = item.mask_id
		local td = sort_td[mask_id]
	...
	local function sort_func(x, y)
		x_data = sort_cached_mask_data[x.mask_id]
		y_data = sort_cached_mask_data[y.mask_id]
		...
		if x_data.dlc ~= y_data.dlc then
			if not x_data.dlc then
				return true
			elseif not y_data.dlc then
				return false
			else
				return x_data.dlc < y_data.dlc
			end
		end

		if x_data.sort_number ~= y_data.sort_number then
			return x_data.sort_number < y_data.sort_number
		end

		if x_data.value == 0 and y_data.value ~= 0 then
			return false
		elseif x_data.value ~= 0 and y_data.value == 0 then
			return true
		end

		if x_data.iso ~= y_data.iso then
			return x_data.iso < y_data.iso
		end

		if x_data.loc ~= y_data.loc then
			return x_data.loc < y_data.loc
		end
	...
	for global_value, gv_items in pairs(items) do
		for _, item in pairs(gv_items) do
			cache_mask_data(item)
		end
...
function BlackMarketGui:mask_mods_callback(data)
	...
	for _, category in pairs(list) do
		...
		for i = 1, #items do
			...
			if not listed_items[items[i].id] and td.texture or td.colors or td.color then
				...
				mods[#mods].pc = td.value or 0
				mods[#mods].color = td.color
			end
		end

		local sort_td = tweak_data.blackmarket[category]
		local x_pc, y_pc, x_sn, y_sn, x_color, y_color = nil
		local x_hsv = {}
		local y_hsv = {}

		table.sort(mods, function (x, y)
			if x.default ~= y.default then
				return x.default
			end

			if x.color and y.color then
				x_color = x.color
				x_hsv.h, x_hsv.s, x_hsv.v = rgb_to_hsv(x_color.r, x_color.g, x_color.b)
				y_color = y.color
				y_hsv.h, y_hsv.s, y_hsv.v = rgb_to_hsv(y_color.r, y_color.g, y_color.b)

				if x_hsv.s == 0 and y_hsv.s ~= 0 then
					return false
				elseif y_hsv.s == 0 and x_hsv.s ~= 0 then
					return true
				end

				if x_hsv.h ~= y_hsv.h then
					return x_hsv.h < y_hsv.h
				end

				if x_hsv.v ~= y_hsv.v then
					return y_hsv.v < x_hsv.v
				end
			end
			...
			x_sn = x.global_value and tweak_data.lootdrop.global_values[x.global_value].sort_number or 0
			y_sn = y.global_value and tweak_data.lootdrop.global_values[y.global_value].sort_number or 0
	...
	new_node_data.extra_options_panel = {
		h = 100,
		on_create_func_name = "populate_customize_mask_info"
	}
	local params = {
...
function BlackMarketGui:choose_mask_part_callback(data)
	...
	if managers.blackmarket:select_customize_mask(category, data.name, data.global_value) then
		self:reload()
	end
end

function BlackMarketGui:populate_customize_mask_info(panel)
	local overwritten_alpha = 0.33
	local mask_mod_info = managers.blackmarket:info_customize_mask()
	local data = {
		panel = ExtendedPanel:new(panel)
	}
	local panel_config = {
		width = panel:h() - 20,
		height = panel:h() - 20
	...
	local material_panel = ExtendedPanel:new(data.panel, panel_config)
	...
	material_text:set_center_x(material_panel:w() / 2)

	local material_bitmap_panel = ExtendedPanel:new(material_panel, {
		y = material_text:bottom(),
	...
	elseif material_data.overwritten then
		icon:set_alpha(overwritten_alpha)
	end

	placer:add_right(material_panel)

	local pattern_panel = ExtendedPanel:new(data.panel, panel_config)
	local pattern_text = FineText:new(pattern_panel, {
		layer = 1,
		w = panel:w(),
		text = managers.localization:to_upper_text("bm_menu_textures"),
		font = small_font,
		font_size = small_font_size,
		color = tweak_data.screen_colors.text
	})

	pattern_text:set_center_x(pattern_panel:w() / 2)

	local pattern_bitmap_panel = ExtendedPanel:new(pattern_panel, {
		y = pattern_text:bottom(),
		h = pattern_panel:h() - pattern_text:bottom()
	})
	local pattern_data = mask_mod_info[2]

	if pattern_data.is_good then
		local pattern_tweak = tweak_data.blackmarket.textures[pattern_data.id]
		local icon = pattern_bitmap_panel:fit_bitmap({
			render_template = "VertexColorTexturedPatterns",
			layer = 1,
			texture = pattern_tweak.texture
		}, pattern_bitmap_panel:size())

		icon:set_center(pattern_bitmap_panel:w() / 2, pattern_bitmap_panel:h() / 2)
	elseif not pattern_data.overwritten then
		local pattern_needed = FineText:new(pattern_bitmap_panel, {
			text = "!",
			layer = 2,
			font = medium_font,
			font_size = medium_font_size,
		...
		pattern_needed:set_center(pattern_bitmap_panel:w() / 2, pattern_bitmap_panel:h() / 2)
	end
...
function BlackMarketGui:choose_mask_color_a_callback(data)
	if managers.blackmarket:select_customize_mask("color_a", data.name, data.global_value) then
		self:reload()
	end
end
...
function BlackMarketGui:show_buy_dlc_callback(data)
	MenuCallbackHandler:open_dlc_store_page(data and data.global_value, "inventory")
end
...
function BlackMarketGui:set_sentry_ap_rounds_callback(data)
	managers.player:set_equipment_setting(data.name, "fire_mode", 2)
	managers.menu_component:post_event("menu_enter")
	self:reload()
end

function BlackMarketGui:set_sentry_default_rounds_callback(data)
	managers.player:set_equipment_setting(data.name, "fire_mode", 1)
	managers.menu_component:post_event("menu_enter")
	self:reload()
end
...
function BlackMarketGui:destroy()
	...
	if self._searchbox then
		self._saved_search = self._searchbox.text:text()

		self._searchbox:destroy()
