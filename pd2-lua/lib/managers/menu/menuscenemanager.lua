...
function MenuSceneManager:_use_environment(name)
	...
	if managers.viewport:default_environment() ~= setting.environment then
		...
		managers.viewport:set_default_environment(setting.environment, nil, )
	end
...
function MenuSceneManager:spawn_grenade_clbk(grenade_id)
	...
	self:_set_item_unit(new_unit, nil, , , , {
		id = grenade_id
	})
...
function MenuSceneManager:spawn_item_weapon(factory_id, blueprint, cosmetics, texture_switches, custom_data)
	...
	self:_set_item_unit(new_unit, nil, , , second_unit, custom_data)
	mrotation.set_yaw_pitch_roll(self._item_rot_mod, 90, 0, 0)
...
function MenuSceneManager:spawn_workbench_room(workbench_name)
	...
	if not managers.dyn_resource:is_resource_ready(ids_unit, ids_unit_workbench_room_name, DynamicResourceManager.DYN_RESOURCES_PACKAGE) then
		print("[MenuSceneManager:spawn_workbench_room]", "workbench room unit is not loaded, force loading it.")
		managers.dyn_resource:load(ids_unit, ids_unit_workbench_room_name, DynamicResourceManager.DYN_RESOURCES_PACKAGE, false)
...
function MenuSceneManager:_test_start_open_economy_safe(safe_entry)
	managers.network.account:inventory_reward_unlock(safe_entry, nil, , callback(self, self, "_safe_result_recieved"))
