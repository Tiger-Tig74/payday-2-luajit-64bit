function WINDLCManager:init_entitlements()
	Global.dlc_manager.all_dlc_data.pd2_clan.entitlement_id = "b00bcc69f3d84d4784974030e5ccb512"
	Global.dlc_manager.all_dlc_data.pd2_clan.external = true
	Global.dlc_manager.all_dlc_data.solus_clan.entitlement_id = "b00bcc69f3d84d4784974030e5ccb512"
	Global.dlc_manager.all_dlc_data.solus_clan.external = true
	Global.dlc_manager.all_dlc_data.raidww2_clan.entitlement_id = "b00bcc69f3d84d4784974030e5ccb512"
	Global.dlc_manager.all_dlc_data.raidww2_clan.external = true
	Global.dlc_manager.all_dlc_data.dbd_clan.entitlement_id = "b00bcc69f3d84d4784974030e5ccb512"
	Global.dlc_manager.all_dlc_data.dbd_clan.external = true
	Global.dlc_manager.all_dlc_data.speedrunners.entitlement_id = "49c924e301054fc88c22c5cd232e573b"
	Global.dlc_manager.all_dlc_data.speedrunners.external = true
	Global.dlc_manager.all_dlc_data.e3_s15a.entitlement_id = "23a0aeacad244f859768aad406d9911e"
	Global.dlc_manager.all_dlc_data.e3_s15a.external = true
	Global.dlc_manager.all_dlc_data.e3_s15b.entitlement_id = "23a0aeacad244f859768aad406d9911e"
	Global.dlc_manager.all_dlc_data.e3_s15b.external = true
	Global.dlc_manager.all_dlc_data.e3_s15c.entitlement_id = "23a0aeacad244f859768aad406d9911e"
	Global.dlc_manager.all_dlc_data.e3_s15c.external = true
	Global.dlc_manager.all_dlc_data.e3_s15d.entitlement_id = "23a0aeacad244f859768aad406d9911e"
	Global.dlc_manager.all_dlc_data.e3_s15d.external = true
	Global.dlc_manager.all_dlc_data.pdcon_2015.entitlement_id = "9397f308ad1c4d7e80d87ee7c726561e"
	Global.dlc_manager.all_dlc_data.pdcon_2015.external = true
	Global.dlc_manager.all_dlc_data.pdcon_2016.entitlement_id = "9397f308ad1c4d7e80d87ee7c726561e"
	Global.dlc_manager.all_dlc_data.pdcon_2016.external = true
	Global.dlc_manager.all_dlc_data.sparkle.entitlement_id = "9397f308ad1c4d7e80d87ee7c726561e"
	Global.dlc_manager.all_dlc_data.sparkle.external = true
	Global.dlc_manager.all_dlc_data.pn2.entitlement_id = "9397f308ad1c4d7e80d87ee7c726561e"
	Global.dlc_manager.all_dlc_data.pn2.external = true
	Global.dlc_manager.all_dlc_data.wcc_s01.entitlement_id = "9397f308ad1c4d7e80d87ee7c726561e"
	Global.dlc_manager.all_dlc_data.wcc_s01.external = true
	Global.dlc_manager.all_dlc_data.wcc_s02.entitlement_id = "9397f308ad1c4d7e80d87ee7c726561e"
	Global.dlc_manager.all_dlc_data.wcc_s02.external = true
	Global.dlc_manager.all_dlc_data.svc.entitlement_id = "9397f308ad1c4d7e80d87ee7c726561e"
	Global.dlc_manager.all_dlc_data.svc.external = true
	Global.dlc_manager.all_dlc_data.ant.entitlement_id = "e30351a32d8048ebb14fff2c9e581431"
	Global.dlc_manager.all_dlc_data.ant.external = true
	Global.dlc_manager.all_dlc_data.sbzac_elegantteeth = {
		no_install = true,
		entitlement_id = "3b33555360c749249cf37923af3121a4",
		external = true
	}
	Global.dlc_manager.all_dlc_data.sbzac_wpn_fps_upg_charm_skullz = {
		no_install = true,
		entitlement_id = "af17b1ef651345d1a8a48ea6ff67fe36",
		external = true
	}
	Global.dlc_manager.all_dlc_data.sbzac_elegantscarf = {
		no_install = true,
		entitlement_id = "10e2783cc2244fcfbf40a316ac84bc55",
		external = true
	}
	Global.dlc_manager.all_dlc_data.sbzac_color_sbzac2_01 = {
		no_install = true,
		entitlement_id = "29bb159f7efb4a8f98103d0648788a51",
		external = true
	}
	Global.dlc_manager.all_dlc_data.rat_oilbaron = {
		no_install = true,
		entitlement_id = "ee24dd9f429f408a879cf417c78f964a",
		external = true
	}
	Global.dlc_manager.all_dlc_data.rat_ranchdiesel = {
		no_install = true,
		entitlement_id = "c4d74be8d4cf41d5a5921c11094a5bdf",
		external = true
	}
	Global.dlc_manager.all_dlc_data.rat_mocow = {
		no_install = true,
		entitlement_id = "8c7aa8537d474094b0f93e1ccf06852d",
		external = true
	}
	Global.dlc_manager.all_dlc_data.prim_primtime = {
		no_install = true,
		entitlement_id = "97480f59f8424e3a89810e270583f468",
		external = true
	}
	Global.dlc_manager.all_dlc_data.prim_darkmat = {
		no_install = true,
		entitlement_id = "271cc010fb0c456db16afbd1ceb94641",
		external = true
	}
	Global.dlc_manager.all_dlc_data.prim_newhorizon = {
		no_install = true,
		entitlement_id = "d0a07cefd5ff443a8b597370bf766bbe",
		external = true
	}
	Global.dlc_manager.all_dlc_data.a11th_homburg = {
		no_install = true,
		entitlement_id = "3bb309cf0f2946ffa9b060afd035eea8",
		external = true
	}
	Global.dlc_manager.all_dlc_data.a11th_homburg_grey = {
		no_install = true,
		entitlement_id = "07af0503ae6b4510b503a4e8d4495f70",
		external = true
	}
	Global.dlc_manager.all_dlc_data.a11th_corl = {
		no_install = true,
		entitlement_id = "837e358f1849463f93c83f2af546ae60",
		external = true
	}
	Global.dlc_manager.all_dlc_data.a11th_stinger = {
		no_install = true,
		entitlement_id = "0160eece867b4b108192e511f84c237d",
		external = true
	}
	Global.dlc_manager.all_dlc_data.trt_railhat = {
		no_install = true,
		entitlement_id = "8a639d555e414f6d96c89daf15412252",
		external = true
	}
	Global.dlc_manager.all_dlc_data.trt_railwork = {
		no_install = true,
		entitlement_id = "caf6162cf483441493bba569e61ed2c5",
		external = true
	}
	Global.dlc_manager.all_dlc_data.trt_railroad = {
		no_install = true,
		entitlement_id = "856324f5178b4cb79670a6fb92cf904c",
		external = true
	}
	Global.dlc_manager.all_dlc_data.h22_deadman = {
		no_install = true,
		entitlement_id = "5068bdffe90f41d99bb3364c698747a0",
		external = true
	}
	Global.dlc_manager.all_dlc_data.h22_nightwalker = {
		no_install = true,
		entitlement_id = "4ae27196dd404d9e88f8491ceab0331e",
		external = true
	}
	Global.dlc_manager.all_dlc_data.h22_tasslefringe = {
		no_install = true,
		entitlement_id = "ad637b9c32754b2f8de6adb6b27402ba",
		external = true
	}
	Global.dlc_manager.all_dlc_data.h22_bloodysnarl = {
		no_install = true,
		entitlement_id = "6ba11d02103d40bb881aeaeee3635b47",
		external = true
	}
	Global.dlc_manager.all_dlc_data.h22_ghostly = {
		no_install = true,
		entitlement_id = "cff914faa08142c9b6c36ff7d00461b2",
		external = true
	}
	Global.dlc_manager.all_dlc_data.h22_tornrags = {
		no_install = true,
		entitlement_id = "3098a3509615457e84ea63d3393163cc",
		external = true
	}
	Global.dlc_manager.all_dlc_data.h22_banshee = {
		no_install = true,
		entitlement_id = "52f194a7fe524b9591988f774b003dd6",
		external = true
	}
	Global.dlc_manager.all_dlc_data.h22_darkprince = {
		no_install = true,
		entitlement_id = "18a43b128ad2440aa6fbaf2ed22fee5f",
		external = true
	}
	Global.dlc_manager.all_dlc_data.h22_devilclaws = {
		no_install = true,
		entitlement_id = "cb43a681f3f447e1b8fec3b6766530f3",
		external = true
	}
	Global.dlc_manager.all_dlc_data.h22_devilhorn = {
		no_install = true,
		entitlement_id = "b724258340d642c299e1589926917522",
		external = true
	}
	Global.dlc_manager.all_dlc_data.cot_smilecigar = {
		no_install = true,
		entitlement_id = "6d27f3b50a1343a39a6efb48f1440480",
		external = true
	}
	Global.dlc_manager.all_dlc_data.cot_sleekygent = {
		no_install = true,
		entitlement_id = "964138fe4a064a1683d7198d525e69c8",
		external = true
	}
	Global.dlc_manager.all_dlc_data.cot_beigedriver = {
		no_install = true,
		entitlement_id = "7825a51f35c647a4be462ece610936cc",
		external = true
	}
	Global.dlc_manager.all_dlc_data.dot_roclown_suit = {
		no_install = true,
		entitlement_id = "aab61c21e7084f6e803623aea9cec8b8",
		external = true
	}
	Global.dlc_manager.all_dlc_data.dot_roclogrip_glv = {
		no_install = true,
		entitlement_id = "cdb007a464e24102be83567475ea486e",
		external = true
	}
	Global.dlc_manager.all_dlc_data.dot_roclown_msk = {
		no_install = true,
		entitlement_id = "49f52f37c68942bab04f41173609e3df",
		external = true
	}
	Global.dlc_manager.all_dlc_data.ztm.entitlement_id = "6dfa406f236f4bdd9ae9cd2b989b190c"
	Global.dlc_manager.all_dlc_data.ztm.external = true
end
