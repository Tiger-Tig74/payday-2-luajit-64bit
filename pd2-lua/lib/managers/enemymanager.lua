...
function EnemyManager:update(t, dt)
	...
	if not self._stopping then
		...
		if self._corpse_disposal_t and self._corpse_disposal_t < t then
			self:_upd_corpse_disposal(t)
		end

		if self._shield_disposal_t and self._shield_disposal_t < t then
...
function EnemyManager:_update_queued_tasks(t, dt)
	...
	if next_callback and next_callback[2] < t then
...
function EnemyManager:on_enemy_died(dead_unit, damage_info)
	...
	if not enemy_u_data[u_key] then
		local u_data = {
			unit = dead_unit
		}
	end
...
function EnemyManager:register_shield(shield_unit)
	...
	if self:shield_limit() < enemy_data.nr_shields then
...
function EnemyManager:_upd_corpse_disposal(t)
	...
	local to_dispose = {}
	local nr_found = 0
	local min_dis_sq = self._MIN_DISPOSAL_DIST_SQ
	local dot_chk = self._DISPOSAL_DOT_CHECK
	...
	if self:corpse_limit() < enemy_data.nr_corpses then
...
function EnemyManager:_upd_shield_disposal(t)
	...
	elseif nr_shields > 0 then
		...
		for u_key, u_data in pairs(shields) do
			if not delay or u_data.death_t < delay then
