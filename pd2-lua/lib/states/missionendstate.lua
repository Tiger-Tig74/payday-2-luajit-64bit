...
function MissionEndState:at_enter(old_state, params)
	...
	if not self._setup then
		...
		managers.hud:load_hud(self.GUI_ENDSCREEN, false, true, false, {}, nil, , true)
	...
	if self._success then
		...
		if not managers.statistics:is_dropin() then
			...
			for achievement, achievement_data in pairs(tweak_data.achievement.complete_heist_achievements) do
				...
				specials_killed_pass = not achievement_data.specials_killed

				if achievement_data.specials_killed then
					specials_killed_pass = true
				...
				used_projectile_pass = not achievement_data.used_projectile

				if achievement_data.used_projectile ~= nil then
					used_projectile_pass = not managers.statistics:session_anyone_used_projectile_except(achievement_data.used_projectile)
				end
